...
// Initialize Gemini outside the component to avoid repeated initialization
const genAI = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY_OLD });
...
export default function Workbench() {
  ...
  const { preferences, selectedAttractions, itinerary, setItinerary, removeItineraryItem, moveItineraryItem, updateItineraryItem, addItineraryItem, saveCurrentTrip, loadedTripName, resetLoadedTripName, unlockSection } = useAppStore();
  ...
  useEffect(() => {
    if (itinerary.length === 0 && selectedAttractions.length > 0 && !isGenerating) {
      const generateAITrip = async () => {
        ...
        resetLoadedTripName();
        try {
          const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY_OLD });
